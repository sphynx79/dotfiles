...
def generate_rofi(binds):
    ...
    for bind in binds:
        ...
        headers = [h.strip() for h in (header1, header2, header3, header4, header5) if str(h).strip()]
        header_block = f" [{'|'.join(headers)}] " if headers else " "

        displayed_rofi_keys = f"{displayed_keys:<20} {delimiter:<5} {header_block} {description}"
